...
object Scaffold : Module("Scaffold", Category.PLAYER, Keyboard.KEY_I, hideModule = false) {
    ...
    // AutoPlace
    private val autoPlaceValue = BoolValue("AutoPlace", false)
    private val autoPlaceDelay = FloatValue("AutoPlaceDelay", 0f, 0f..4f) { autoPlaceValue.get() }
    private val autoPlaceMouseDown = BoolValue("AutoPlaceMouseDown", false) { autoPlaceValue.get() }
    private val autoPlaceClickTimer = MSTimer()
    private var autoPlaceClickCount = 0
    private var autoPlaceLastMovingObject: MovingObjectPosition? = null
    private var autoPlaceLastBlockPos: BlockPos? = null
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        // AutoPlace logic
        if (autoPlaceValue.get()) {
            if (mc.currentScreen != null || player.capabilities.isFlying) return
            ...
            if (autoPlaceLastMovingObject != null && autoPlaceClickCount < autoPlaceDelay.get()) {
                autoPlaceClickCount++
                return
            }

            autoPlaceLastMovingObject = movingObject
            val blockPos = movingObject.blockPos

            if (autoPlaceLastBlockPos?.let { pos ->
                    pos.x == blockPos.x && pos.y == blockPos.y && pos.z == blockPos.z
                } == true) return

            val block = mc.theWorld.getBlockState(blockPos).block
            if (block == null || block == Blocks.air || block is BlockLiquid || (autoPlaceMouseDown.get() && !Mouse.isButtonDown(1))) return

            if (autoPlaceClickTimer.hasTimePassed(25)) {
                tryToPlaceBlock(itemStack, blockPos, movingObject.sideHit, movingObject.hitVec)
                autoPlaceLastBlockPos = blockPos
                autoPlaceClickCount = 0
                autoPlaceClickTimer.reset()
            }
        }
                ...
                for (yaw in list) {
                    for (pitch in pitchList step 0.1) {
                        ...
                        if (raytrace.blockPos == neighbor && raytrace.sideHit == side.opposite) {
                            ...
                            // The module should be looking to aim at (nearly) the upper face of the block. Provides stableBridging most of the time.
    ...
    private fun tryToPlaceBlock(
        ...
        if (autoPlaceValue.get()) {
            // AutoPlace logic
            // Гарантированно отключаем левую кнопку мыши
            MouseUtils.setMouseButtonState(0, false)

// Активируем правую кнопку мыши (для строительства)
            MouseUtils.setMouseButtonState(1, true)

            val clickedSuccessfully = thePlayer.onPlayerRightClick(clickPos, side, hitVec, stack)
            thePlayer.swingItem()
            mc.effectRenderer.addBlockHitEffects(clickPos, side)

// Отпускаем правую кнопку мыши
            MouseUtils.setMouseButtonState(1, false)

// Повторно отключаем левую кнопку мыши на всякий случай
            MouseUtils.setMouseButtonState(0, false)
            ...
            if (clickedSuccessfully) {
                if (!attempt) {
                    ...
                    autoPlaceClickTimer.reset()
                    autoPlaceClickCount = 0
                    autoPlaceLastBlockPos = clickPos
